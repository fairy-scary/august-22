...
***********************************************************************/
/* PLAN:
-TAKES IN ARRAY OF STRINGS(data types)

- RETURN FUNCTION(CLOSURE):
  - accepts STRING that's also a type
  - for all eles, IF type ===, don't add to new array ....use filter?
  - return new array */
...
function eliminateType(arr) {
  ...
  return (type) => {
    ...
    for(let i = 0; i < arr.length; i++) {
      if( typeof arr[i] !== type) newArr.push(arr[i])
    }
    return newArr
  }
